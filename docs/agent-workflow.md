...
- `resolve_thread`
- `request_next_round`
- `review.get_state` _(stub — registered on the surface but not yet implemented; always returns `kind: "not_implemented"`)_
- `review.list_actions` _(stub — registered on the surface but not yet implemented; always returns `kind: "not_implemented"`)_
